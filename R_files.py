...
###################################################################################
    def __init__(self,simulname=None,time=None, floattype=float, light = False,\
        ...
        if self.output: print('simulname is',simulname)
        ...
        self.ncname=files(self.simul, time=self.time, output = self.output)
        ...
        if self.ncname.model in ['ucla','croco']:
            ...
            time1 = self.time - (self.time-self.ncname.shift)%self.ncname.tfile
            ...
            if 'rrexnum' not in self.simul and\
                'brazil' not in self.simul and\
                'eurec1' not in self.simul: date2 = date2 -timedelta(days=1)
            ###############
            ...
            if (self.simul == 'gigatl3_6h' and self.time>=1550)\
                 ...
                 or (self.simul == 'gigatl3_1h_avg' and self.time>=669)\
                 or (self.simul == 'gigatl3_1h' and self.time>=6690):
                if self.output: print('fix for some GIGATL3/6 simulations')
        ...
        if touchfile:
            try:
                ...
                if self.output: print("no time in file")


        try:
            if touchfile:
                if self.output: print('coord')
                self.coord=self.get_domain(self.ncfile,self.ncname.grd,self.domain,\
                                           time,output = output)
        
                if self.output: print('coordmax')
                self.coordmax=self.get_domain(self.ncfile, self.ncname.grd,\
                            '[0,1e9,0,1e9,[1,1e9,1]]', time ,output = output)
                
                if self.output: print('cst')
                self.cst();
                
                if self.output: print('dt')
                self.dt()
                
            else:
                self.coord=[0,-1,0,-1,[0]]
                if self.output: print(self.ncfile)
                if self.output: print("not touching _his file, loading _grd anyway")
        except:
            self.coord=[0,-1,0,-1,[0]]
            if self.output: print(self.ncfile)
            if self.output: print("no _his file, loading _grd anyway")
        ...
        try:
            ...
            if light:
                [self.mask,self.pm,self.pn,self.x,self.y] = self.variables_grd(light=True)
            elif light_bulk:
                [self.mask,self.x,self.y,self.angle] = self.variables_grd(light_bulk=True)
            else:
                [self.topo,self.mask,self.pm,self.pn,self.f,self.x,self.y,self.angle] =\
                      self.variables_grd()
...
###################################################################################
    def update(self,time=None):
        ...
        if self.ncname.model in ['ucla','croco']:
            ...
            #fix for some simulations where XIOS name is shifted by one day.
            if (self.simul == 'gigatl3_6h' and self.time>=1550)\
                 or 'gigatl6_1h_tides' in self.simul\
                 or (self.simul == 'gigatl3_1h_avg' and self.time>=669)\
                 or (self.simul == 'gigatl3_1h' and self.time>=6690):
            ...
            if self.output: print('self.time is ',self.time)
        ...
        try:
            ...
            if self.output: print("no oceantime in file")
...
example: for an interactive session:
         load(simul = 'filam [0,1000,0,1600,[-100,0,10]] 190')
            ...
            self.ncname=files(self.simul, time=self.time0, output = self.output)
...
###################################################################################
    ...
    @staticmethod
    def get_domain(ncname,ncname0,domainname,time,output=False,*args):

        if output: print('loading', ncname0)
        ncfile0 = Dataset(ncname0, 'r')
        if output: print('file opened in get_domain')
            ...
            if len(eval(domainname)[4])==1:
              ...
              try:
                if ncname!=ncname0:
                    ...
                    ncfile.close()
                    if output: print('file closed in get_domain')
                else:
                    depths=np.arange(eval(domainname)[4][0],int(np.min([len(ncfile0.dimensions['s_rho']),eval(domainname)[4][1]]))+1,eval(domainname)[4][2])
        ...
        try:
            ...
            # for XIOS outputs
            nx2 = int(np.min([nx2,len(ncfile0.dimensions['x'])]))
            ny2 = int(np.min([ny2,len(ncfile0.dimensions['y'])]))
...
###################################################################################
    ...
    def variables_grd(self, light = False, light_bulk = False):

        if self.output: print(self.coord)

        [ny1,ny2,nx1,nx2] = self.coord[0:4]
        #ncfile0 = NetCDFFile(ncname,'r')
        ncfile0 = Dataset(self.ncname.grd, 'r')
        if self.output: print('file opened in variables_grd')
        if self.output: print('ncname0,ny1,ny2,nx1,nx2')
        if self.output: print(self.ncname.grd,ny1,ny2,nx1,nx2)
        ...
        try:
            ...
            if not light and not light_bulk:
            ...
            ncfile0.close()
            if self.output: print('file closed in variables_grd')
            
            if self.output: print('[topo,pm,pn,f,lat,lon] have just been loaded')
            if self.output: print('----------------------------------------------------------')
            if self.output: print('All arrays are now Fortran ordered and indices are [i,j,k]')
            if self.output: print('----------------------------------------------------------')

            if self.output and not light and not light_bulk: print(topo.shape)
            
            if light:
                return [mask,pm,pn,lon,lat]
            elif light_bulk:
                return [mask,lon,lat,angle]
            else:
...
###################################################################################
    ...
    def cst(self):
        ...
        try:
            ...
            if self.output: print('file opened in cst')
        except:
            if self.output: print('cannot find: ', self.ncfile)
        ...
        try:
            ...
            if self.output: print('read Cs_r in ncfile.variables')
        except:
            try:
                ...
                if self.output: print('read Cs_r in ncfile.Cs_r')
                ...
                try:
                    ...
                    try:
                        ...
                        if self.output: print('read Cs_r in grdfile.variables')
                        grdfile.close()
                    except:
                        try:
                            self.Cs_r = self.Forder(grdfile.Cs_r)
                            self.Cs_w = self.Forder(grdfile.Cs_w)
                            if self.output: print('read Cs_r in grdfile.Cs_r')
                            grdfile.close()
                        except:
                            grdfile.close()
                except:
                    if self.output: print('cannot open grdfile to get Cs_r,Cs_w')
        ...
        try:
            ...
            if self.output: print('no Zob in job ... using Zob = 0.01')
            self.Zob = 0.01
        ...
        ncfile.close()
        if self.output: print('file closed in cst')
...
###################################################################################
    ...
    def oceandate(self):
        ...
        ncfile = Dataset(self.ncfile, 'r')
        if self.output: print('file opened in oceandate')
        try:
            ...
            try:
                ...
                try:
                    self.oceantime = int(np.array(ncfile.variables['time_counter'][self.infiletime]))
                except:
                    try:
        ...
        ncfile.close()
        if self.output: print('file closed in oceandate')
...
###################################################################################
    ...
    def dt(self):
        ...
        ncfile = Dataset(self.ncfile, 'r')
        if self.output: print('file opened in dt')
...
###################################################################################
    ...
    def __init__(self, simul, time=0, output =True):
        ...
        #ncra atlbig_dhis.00[7-9]* atlbig_dhis.0[1-2]* atlbig_dhis.mean2.nc
        elif simul=='atlbig_uv_mean2':
            #folder='/mnt/inca/gula'
            #folder=shirma + '/gula/ROMS/Simulations'
            folder=ROMSSIMSK
            self.his=folder + '/ATLBIG/Energy/atlbig_energy_components_surf_uv.mean2.'
            self.grd=folder + '/ATLBIG/atlbig_grd.nc'
            self.frc=folder + '/ATLBIG/atlbig_frc.nc'
            self.wind=folder + '/ATLBIG/atlbig_frc_wind.nc'
            self.tfile=1
...
#temporary################################################################
        ...
        # clim_seas gulfz_his.00[3-9]* gulfz_his.0[1-2]* gulfz_his.03[0-8]*
            ...
            self.his=ROMSSIMS+'/NESEA/HIS_original/nesea_his.'
        ...
        ##################
            ...
            if os.getenv('HOSTNAME') is None:
                ...
                self.grd='/home/datawork-lemar-apero/jgula/POLGYR/polgyr_grd.nc'
        ...
        ################## Test case (to test online features)
        ...
        elif 'gulfstream' in simul and 'gigatl' not in simul:
            ...
            if 'mycroco' in simul:
                folder = libra + '/gula/ROMS/test_roms/test_budget/test_budget_gulfstream_mycroco'
                ...
                folder = libra + '/gula/ROMS/test_roms/test_budget/test_budget_gulfstream'
            ...
            if 'avg' in simul:
                if 'vrt' in simul:
                    ...
                    self.his = folder + out + '/croco_diags_wdia_avg'
                else:
                ...
                if 'vrt' in simul:
                    ...
                    self.his = folder + out + '/croco_diags_wdia'
                else:
        ...
        ################## Test case (to test online features)
            ...
            if 'croco22' in simul:
                folder_config = folder_code +'/Config_croco22'
            else:
                folder_config = folder_code +'/Config_croco'
                
            if 'SW' in simul:
                if 'noS' in simul: 
                    if 'wind' in simul and 'nodiff' in simul:    
                        folder = folder_config + '_SW_GLS_wind_nodiff_noS'
                    else:
                        folder = folder_config + '_SW_GLS_noS'
                elif 'large' in simul:                 
                    folder = folder_config + '_SW_GLS_large'
                elif 'dz' in simul and 'GLS' in simul:                 
                    folder = folder_config + '_SW_GLS_dz'
                elif 'HHR' in simul:
                    if 'KPP0' in simul:
                        if '_U' in simul:
                            if 'N' in simul:
                                folder_config + '_SW_N_KPP0_HHR_U/case1'
                elif 'VHR' in simul:
                    if 'KPP0' in simul:
                        if '_U' in simul:
                            if 'N' in simul:
                                folder_config + '_SW_N_KPP0_VHR_U/case1'
                    elif 'GLS' in simul:    
                        folder = folder_config + '_SW_GLS_VHR'
                    else:               
                        folder = folder_config + '_SW_VHR'
                elif 'HRzz' in simul:
                    if 'KPP0' in simul:    
                        if '_U' in simul: 
                            if 'N' in simul:
                                folder = folder_config + '_SW_N_KPP0_HRzz_U/case1'
                                #folder = '/home2/scratch/jgula/Seamount/Config_croco_SW_N_KPP0_VHR_U'
                elif 'HRz' in simul:
                    if 'KPP0' in simul:
                        if '_U' in simul:
                            if 'N' in simul:
                                folder = folder_config + '_SW_N_KPP0_HRz_U/case1'
                                #folder = '/home2/scratch/jgula/Seamount/Config_croco_SW_N_KPP0_VHR_U'
                elif 'HR' in simul:
                    if 'GLSD' in simul:    
                        if '_U' in simul: 
                            if 'N' in simul:
                                folder = folder_config + '_SW_N_GLSD_HR_U/case1'
                                #folder = '/home2/scratch/jgula/Seamount/Config_croco_SW_N_GLSD_HR_U'
                            else:
                                folder = folder_config + '_SW_GLSD_HR_U/case1'
                    elif 'GLS0' in simul:    
                        if '_U' in simul: 
                            folder = folder_config + '_SW_GLS0_HR_U/case1'
                    elif 'KPP0D' in simul:    
                        if '_U' in simul: 
                            if 'N' in simul:
                                folder = folder_config + '_SW_N_KPP0D_HR_U/case1'
                                #folder = '/home2/scratch/jgula/Seamount/Config_croco_SW_N_KPP0D_HR_U'
                    elif 'KPPDNOKT' in simul:    
                        if '_U' in simul: 
                            if 'N' in simul:
                                folder = folder_config + '_SW_N_KPPDNOKT_HR_U/case1'
                                #folder = '/home2/scratch/jgula/Seamount/Config_croco_SW_N_KPPDNOKT_HR_U'
                    elif 'KPPD' in simul:    
                        if '_U' in simul: 
                            if 'N' in simul:
                                folder = folder_config + '_SW_N_KPPD_HR_U/case1'
                                #folder = '/home2/scratch/jgula/Seamount/Config_croco_SW_N_KPPD_HR_U'
                    elif 'KPP0DNOKT' in simul:    
                        if '_U' in simul: 
                            if 'N' in simul:
                                folder = folder_config + '_SW_N_KPP0DNOKT_HR_U/case1'
                                #folder = '/home2/scratch/jgula/Seamount/Config_croco_SW_N_KPP0DNOKT_HR_U'
                    elif 'KPP0' in simul:    
                        if '_U' in simul: 
                            if 'N' in simul:
                                folder = folder_config + '_SW_N_KPP0_HR_U/case1'
                                #folder = '/home2/scratch/jgula/Seamount/Config_croco_SW_N_KPP0_HR_U'
                            else:
                                folder = folder_config + '_SW_KPP0_HR_U/case1'
                        else:          
                            folder = folder_config + '_SW_KPP_HR'
                    elif 'KPPNOR' in simul:    
                        if '_U' in simul: 
                            folder = folder_config + '_SW_KPPNOR_HR_U/case1'
                    elif 'KPP' in simul:    
                        if '_U' in simul: 
                            folder = folder_config + '_SW_KPP_HR_U/case1'
                    elif 'GLS' in simul:    
                        if '_U' in simul: 
                            if 'N' in simul:
                                folder = folder_config + '_SW_N_GLS_HR_U/case1'
                            else:
                                folder = folder_config + '_SW_GLS_HR_U/case1'
                        else:          
                            folder = folder_config + '_SW_GLS_HR'
                    else:   
                        if '_U' in simul:    
                            folder = folder_config + '_SW_HR_U/case1'
                        else:          
                            folder = folder_config + '_SW_HR'
                    ...
                    if 'GLS' in simul:
                        if 'nodiff' in simul:
                            if 'vadvc2' in simul:
                                if 'hadvc2' in simul:
                                    folder = folder_config + '_SW_GLS_wind_nodiff_vadvc2_hadvc2'
                                elif 'hadvc4' in simul:
                                    if 'test' in simul:
                                        folder = folder_config + '_SW_GLS_wind_nodiff_vadvc2_hadvc4_test'
                                    elif 'dif4' in simul:
                                        if 'vis4' in simul:
                                            folder = folder_config + '_SW_GLS_wind_nodiff_vadvc2_hadvc4_dif4_vis4'
                                        else:
                                            folder = folder_config + '_SW_GLS_wind_nodiff_vadvc2_hadvc4_dif4'
                                    else:
                                        folder = folder_config + '_SW_GLS_wind_nodiff_vadvc2_hadvc4'
                                elif 'hadvweno5' in simul:
                                    folder = folder_config + '_SW_GLS_wind_nodiff_vadvc2_hadvweno5'
                                elif 'vis4' in simul:
                                    folder = folder_config + '_SW_GLS_wind_nodiff_vadvc2_vis4'
                                elif 'testv3' in simul:
                                    folder = folder_config + '_SW_GLS_wind_nodiff_vadvc2_testv3'
                                elif 'testv2' in simul:
                                    folder = folder_config + '_SW_GLS_wind_nodiff_vadvc2_testv2'
                                elif 'test' in simul:
                                    folder = folder_config + '_SW_GLS_wind_nodiff_vadvc2_test'
                                else:
                                    folder = folder_config + '_SW_GLS_wind_nodiff_vadvc2'
                            elif 'vadvweno5' in simul:
                                folder = folder_config + '_SW_GLS_wind_nodiff_vadvweno5'
                            else:
                                folder = folder_config + '_SW_GLS_wind_nodiff'
                        else:
                            if 'hadvup3' in simul:
                                folder = folder_config + '_SW_GLS_wind_hadvup3'
                            else:
                                folder = folder_config + '_SW_GLS_wind'
                    else:
                        if 'nodiff' in simul:
                            if 'vadvc2' in simul:
                                if 'hadvc2' in simul:
                                    if 'testv2' in simul:
                                        folder = folder_config + '_SW_KPP_wind_nodiff_vadvc2_hadvc2_testv2'
                                    else:
                                        folder = folder_config + '_SW_KPP_wind_nodiff_vadvc2_hadvc2'
                        else:
                            folder = folder_config + '_SW_KPP_wind'
                elif 'GLS' in simul:             
                    folder = folder_config + '_SW_GLS'
                elif 'drag' in simul:             
                    folder = folder_config + '_SW_drag'
                elif 'clean' in simul:             
                    folder = folder_config + '_SW_clean'
                elif 'dz' in simul:             
                    folder = folder_config + '_SW_dz'
                else:
                    folder = folder_config + '_SW'
            else:
                if 'nodiff' in simul:
                    folder = folder_config + '_GLS_nodiff'
                elif 'flat' in simul:
                    folder = folder_config + '_GLS_flat'
                else:
                    folder = folder_config + '_GLS'
        ...
        ##################
            ...
            if 'mean' in simul:
                ...
                if 'vrt' in simul:
                    ...
                    if 'var' in simul:
                        self.his = folder + '/nesev_his.meanvaruv.00500-01399'
                    else:
                        self.his = folder + '/nesev_his.mean.00500-01399'
                elif 'his800' in simul:
                    if 'var' in simul:
                        self.his = folder + '/nesev_his.meanvaruv.00800-00849'
                    else:
                        self.his = folder + '/nesev_his.mean.00800-00849'
                elif 'avg33' in simul:
        ...
        elif simul=='luckyt200':

            self.model = 'croco'
            self.digits = 5
            try:
                if 'curie' in os.getenv('HOSTNAME') or 'irene' in os.getenv('HOSTNAME'):
                    folder='/ccc/store/cont003/gen7638/crystleg/LUCKYT200/'
                    self.grd=folder + 'luckyt200_grd.nc'
                    self.his=folder + 'HIS/luckyt200_his.'
                    self.tstart=0
            except:
                ...
                self.tstart=1000
            self.frc=folder + 'luckyt200_frc.nc'
            self.wind=folder + 'luckyt200_frc.nc'
            self.tfile=4
            self.tend=1376
        ...
        ##################
            ...
            try:
                if 'curie' in os.getenv('HOSTNAME') or 'irene' in os.getenv('HOSTNAME'):
                    folder='/ccc/store/cont003/gen7638/crystleg/LUCKYT50_256/'
                    self.grd=folder + 'luckyt50_grd.nc'
                    self.his=folder + 'HIS/luckyt50_his.'
                    self.tstart=0
            except:
                folder= libra +'/gula/ROMS/Simulations/LUCKYT/'
                self.grd=folder + 'lucky_grd.nc'
            ...
            self.frc=folder + 'luckyt50_frc.nc'
            self.wind=folder + 'luckyt50_frc.nc'
            self.tfile=4
            self.tend=128
        ...
        ##################
            ...
            if 'mean' in simul:
                ...
                year=''.join([n for n in simul if n.isdigit()])
                if len(year)>=2: add = '.year.' + year[-2:]
                else: add = '.mean'
                self.digits = 0
                
                if 'uv' in simul:
                    self.his=folder + 'uv/' + simulname + '_energy_components_uv' + add
                else:
                    self.his=folder + simulname + '_avg' + add

            elif 'avg' in simul:
                self.his=folder + simulname + '_avg.'
            else:
                self.his=folder + simulname + '_his.'
        ...
        elif 'gigatl1' in simul and 'region' in simul:
        ...
        ##################
            ...
            if 'gigatl1_1h_tides_kpp' in simul:
                folder_name= 'GIGATL1_1h_tides_kpp/'
            elif 'gigatl1_1h_tides' in simul:
            ...
            try:
                if 'curie' in os.getenv('HOSTNAME') or 'irene' in os.getenv('HOSTNAME'):
                    ...
                    folder_para = '/home/datawork-lops-megatl/GIGATL1/GRD3'
                    self.grd= '/home/datawork-lops-megatl/GIGATL1/gigatl1_grd_masked.nc'
            except:
                ...
                folder= '/net/omega/local/tmp/2/gula/GIGATL1/' + folder_name
                #folder_para = libra +'/gula/ROMS/Simulations/GIGATL/GIGATL1/INIT_N100_100_100/GRD3'
                folder_para = '/net/omega/local/tmp/2/gula/GIGATL1/GRD3'
                self.grd= '/net/omega/local/tmp/2/gula/GIGATL1/gigatl1_grd.nc'
            ...
            if 'para' in  simul:
                ...
                self.grd= folder_para + '/gigatl1_grd_masked.' +  tile + '.nc'
                ...
                if 'hourly' in simul:
                    ...
                    if 'surf' in simul:
                        self.his = folder + 'HIS_1h/gigatl1_surf.' + tile + '.'
                        self.tfile=120
                    else:
                ...
                if 'surf' in simul:
                    ...
                    if 'eke' in simul and 'mean' in simul:
            
                        year=''.join([n for n in simul if n.isdigit()])
                        if len(year)>=4: add = '_' + year[-4:]
                        else: add = ''

                        self.his = folder + 'SURF/eke/gigatl1_surf_eke.mean' + add
                        ...
                        if 'tides' not in simul:
                            self.shift=2*24 # names are shifted by 2 days
        ...
        ##################        
            ...
            try:
                if 'curie' in os.getenv('HOSTNAME') or 'irene' in os.getenv('HOSTNAME'):
                    folder='/ccc/work/cont003/gen7638/gulaj/Post/GIGATL/EDDY/iso'
            ...
            if 'gigatl3_1h_UP5' in simul:
                self.his = folder + '/gigatl3_1h_UP5_isopycnal_section.'
                self.dtfile=12*3600
                self.tfile=10
                self.tend=100000
            elif 'gigatl3_1h' in simul:
                self.his = folder + '/gigatl3_1h_isopycnal_section.'
                self.dtfile=12*3600
                self.tfile=10
                self.tend=100000
       ...
       ##################
        ...
        elif 'gigatl3' in simul and 'eurec4a' in simul:
            ...
            if os.getenv('HOSTNAME') is None: # libra or LOPS computers
                folder= '/net/omega/local/tmp/2/gula/GIGATL3/zoom_for_eurec4a/'
            else: #datarmor
                folder= '/home/datawork-lops-megatl/Eurec4a/'
                
            self.grd= folder + 'gigatl3_1h_avg_eurec4a.00240.nc'
                
                
            if 'avg' in simul:
            
                self.dtfile=3600*24*5
                self.tfile=1
                    
                basename = 'gigatl3_1h'
                
                if 'tides' in simul:
                    basename = basename + '_tides'
                    
                if 'vrt' in simul:
                    self.his = folder + basename  + '_vrt_eurec4a.'
                elif 'ek' in simul:
                    self.his = folder + basename  + '_ke_eurec4a.'
                elif 'eddy' in simul:
                    self.his = folder + basename  + '_eddy_eurec4a.'
                elif 'uv' in simul:
                    self.his = folder + basename  + '_uv_eurec4a.'
                elif 'ts' in simul:
                    self.his = folder + basename  + '_ts_eurec4a.'
                elif 'pv' in simul:
                    self.his = folder + basename  + '_pv_eurec4a.'
                else:
                    self.his = folder + basename  + '_avg_eurec4a.'

            else:
            
                if 'tides' in simul:
                    self.his = folder + '/gigatl3_1h_tides_hourly_eurec4a.'
                    self.tfile=120
                    self.dtfile=3600
                else:
                    self.his = folder + '/gigatl3_1h_hourly_eurec4a.'
                    self.tfile=120
                    self.dtfile=3600
       ...
       ##################
            ...
            folder= '/net/omega/local/tmp/2/gula/GIGATL3/zoom_for_seamount/'
            self.grd= '/net/omega/local/tmp/2/gula/GIGATL3/zoom_for_seamount/gigatl3_1h_tides_avg_equator.00500.nc'
       ...
       ##################
        ...
        elif 'gigatl3' in simul and 'rrexnum' in simul:
            ...
            folder= '/home/datawork-lops-megatl/rrexnum/'
            self.grd= '/home/datawork-lops-megatl/rrexnum/gigatl3_1h_rrexnum.02890.nc'

            self.his = folder + '/gigatl3_1h_rrexnum.'
            self.dtfile=12*3600
            self.tfile=10
            self.tend=100000
       ...
       ##################
            ...
            if 'gigatl3_1h_UP3_KPP_tides_soda342_mon' in simul:
                folder_name= 'GIGATL3_1h_UP3_KPP_tides_soda342_mon/'
            elif 'gigatl3_1h_UP3_KPP_soda342_mon' in simul:
                folder_name= 'GIGATL3_1h_UP3_KPP_soda342_mon/'
            elif 'gigatl3_1h_tides' in simul:
            ...
            try:
                if 'curie' in os.getenv('HOSTNAME') or 'irene' in os.getenv('HOSTNAME'):
                    ...
                    folder='/home/datawork-lops-megatl/GIGATL3/' + folder_name
                    folder_avg = folder + 'AVG/'
                    self.grd= '/home/datawork-lops-megatl/GIGATL3/gigatl3_grd.nc'

            except:
                folder= '/net/omega/local/tmp/2/gula/GIGATL3/' + folder_name
                folder_avg = folder
            ...
            if 'mean' in simul:
                ...
                if 'vrt' in simul or 'VRT' in simul:
                    self.his = folder + '/GIGATL3_5d_VRT' + add + '.mean'
                elif 'KE' in simul or 'ke' in simul:
                    self.his = folder + '/GIGATL3_5d_KE' + add + '.mean'
                elif 'UV' in simul or 'uv' in simul:
                    self.his = folder + '/GIGATL3_5d_M' + add + '.mean'
                elif 'TS' in simul or 'ts' in simul:
                    self.his = folder + '/GIGATL3_5d_TS' + add + '.mean'
                elif 'EDDY' in simul or 'eddy' in simul:
                    self.his = folder + '/GIGATL3_5d_EDDY' + add + '.mean'
                else:
                    self.his = folder_avg + '/GIGATL3_5d_aver' + add + '.mean'
                self.dtfile=1
                self.tfile=1
                self.digits = 0
                self.model = 'croco'
                ...
                if 'gigatl3_1h_tides' in simul:
                    self.his = folder + 'HIS/uvbot/gigatl3_1h_tides_uvbotz.'
                    self.dtfile=3*3600
                    self.tfile=40
                elif 'gigatl3_1h' in simul:
                    if time < 2330:
                        self.his = folder + 'HIS/uvbot/gigatl3_1h_UP5_uvbotz.'
                    else:
                        self.his = folder + 'HIS/uvbot/gigatl3_1h_uvbotz.'
                    self.dtfile=12*3600
                    self.tfile=10
                ...
                if 'gigatl3_1h_tides' in simul:
                    self.his = '/net/omega/local/tmp/2/gula/GIGATL3/horizontal_section/gigatl3_1h_tides_horizontal_section.'
                    self.dtfile=3*3600
                    self.tfile=40
                elif 'gigatl3_1h' in simul:
                    if time < 2330:
                        self.his = '/net/omega/local/tmp/2/gula/GIGATL3/horizontal_section/gigatl3_1h_UP5_horizontal_section.'
                    else:
                        self.his = '/net/omega/local/tmp/2/gula/GIGATL3/horizontal_section/gigatl3_1h_horizontal_section.'
                    self.dtfile=12*3600
                    self.tfile=10
       ...
       ##################        
            ...
            elif 'ratlbig_new_interz_online_gigatl6' in simul:
                folder_name= 'RATLBIG_new_interz_online_gigatl6/'
            elif 'ratlbig_new_interz_online' in simul:
                folder_name= 'RATLBIG_new_interz_online/'
            elif 'ratlbig_new_interz' in simul:
                folder_name= 'RATLBIG_new_interz/'
            elif 'ratlbig_new_inter' in simul:
                folder_name= 'RATLBIG_new_inter/'
            elif 'ratlbig_new' in simul:
                folder_name= 'RATLBIG_new/'
            elif 'gigatl6_1h_UP3_KPP_ratlbig' in simul:
                self.realyear_tstart = datetime(2004,1,15)
                folder_name= 'GIGATL6_1h_UP3_KPP_ratlbig/'
            elif 'gigatl6_6h_UP3_KPP' in simul:
                self.realyear_tstart = datetime(2004,1,15)
                folder_name= 'GIGATL6_6h_UP3_KPP/'
            ...
            elif 'gigatl6_1h_UP3_noHB_clim' in simul:
                self.realyear_tstart = datetime(2004,1,15)
                folder_name= 'GIGATL6_1h_UP3_noHB_clim/'
            elif 'gigatl6_1h_UP3_clim' in simul:
                self.realyear_tstart = datetime(2004,1,15)
                folder_name= 'GIGATL6_1h_UP3_clim/'
            elif 'gigatl6_1h_UP3_GEO' in simul:
                self.realyear_tstart = datetime(2004,1,15)
                folder_name= 'GIGATL6_1h_UP3_GEO/'
            elif 'gigatl6_1h_UP3_KPP_noCFB' in simul:
                self.realyear_tstart = datetime(2004,1,15)
                folder_name= 'GIGATL6_1h_UP3_KPP_noCFB/'
            elif 'gigatl6_1h_filt_UP3_KPP' in simul:
                self.realyear_tstart = datetime(2004,1,15)
                folder_name= 'GIGATL6_1h_filt_UP3_KPP/'
            elif 'gigatl6_1h_UP3_KPP_noHB_Zob' in simul:
                self.realyear_tstart = datetime(2004,1,15)
                folder_name= 'GIGATL6_1h_UP3_KPP_noHB_Zob/'
            elif 'gigatl6_1h_UP3_KPP_noHB_dSdt' in simul:
                self.realyear_tstart = datetime(2004,1,15)
                folder_name= 'GIGATL6_1h_UP3_KPP_noHB_dSdt/'
            elif 'gigatl6_1h_UP3_KPP_noHB' in simul:
                self.realyear_tstart = datetime(2004,1,15)
                folder_name= 'GIGATL6_1h_UP3_KPP_noHB/'
            elif 'gigatl6_1h_UP3_KPP_noseaice' in simul:
                self.realyear_tstart = datetime(2004,1,15)
                folder_name= 'GIGATL6_1h_UP3_KPP_noseaice/'
            elif 'gigatl6_1h_UP3_KPP_soda342_mon_master' in simul:
                self.realyear_tstart = datetime(2004,1,15)
                folder_name= 'GIGATL6_1h_UP3_KPP_soda342_mon_master/'
            elif 'gigatl6_1h_UP3_KPP_soda342_mon' in simul:
                self.realyear_tstart = datetime(2004,1,15)
                folder_name= 'GIGATL6_1h_UP3_KPP_soda342_mon/'
            elif 'gigatl6_1h_UP3_KPP' in simul:
                self.realyear_tstart = datetime(2004,1,15)
                folder_name= 'GIGATL6_1h_UP3_KPP/'
            ...
            elif 'gigatl6_1h_UP5_KPP_soda342_mon_nobstress' in simul:
                self.realyear_tstart = datetime(2004,1,15)
                folder_name= 'GIGATL6_1h_UP5_KPP_soda342_mon_nobstress/'
            elif 'gigatl6_1h_UP5_KPP_soda342_mon_master_corrected' in simul:
                self.realyear_tstart = datetime(2004,1,15)
                folder_name= 'GIGATL6_1h_UP5_KPP_soda342_mon_master_corrected/'
            elif 'gigatl6_1h_UP5_KPP_soda342_mon_master' in simul:
                self.realyear_tstart = datetime(2004,1,15)
                folder_name= 'GIGATL6_1h_UP5_KPP_soda342_mon_master/'
            elif 'gigatl6_1h_UP5_KPP_soda342_mon' in simul:
                self.realyear_tstart = datetime(2004,1,15)
                folder_name= 'GIGATL6_1h_UP5_KPP_soda342_mon/'
            elif 'gigatl6_1h_tides' in simul:
                self.realyear_tstart = datetime(2004,1,15)
                folder_name= 'GIGATL6_1h_tides/'
            elif 'gigatl6_1h_UP5' in simul:
                self.realyear_tstart = datetime(2004,1,15)
                folder_name= 'GIGATL6_1h_UP5/'
            ...
            try:
                if 'curie' in os.getenv('HOSTNAME') or 'irene' in os.getenv('HOSTNAME'):
                    ...
                    if 'atlbig' not in simul:
                        self.grd= '/home/datawork-lops-megatl/GIGATL6/gigatl6_grd_masked.nc'
                    else:
                        self.grd= '/home/datawork-lops-megatl/GIGATL6/ratlbig_grd.nc'


            except:
                folder= '/net/omega/local/tmp/2/gula/GIGATL6/' + folder_name + 'HIS/'
                if 'noHB' in simul:
                    self.grd= libra +'/gula/ROMS/Simulations/GIGATL/GIGATL6/gigatl6_grd_masked_noHB.nc'
                elif 'atlbig' not in simul or 'gigatl6_1h_UP3_KPP_ratlbig' in simul:
                    self.grd= libra +'/gula/ROMS/Simulations/GIGATL/GIGATL6/gigatl6_grd_masked.nc'
                else:
                    self.grd= libra +'/gula/ROMS/Simulations/RATLBIG/ratlbig_grd.nc'
            ...
            if 'surf' in simul:
                ...
                year=''.join([n for n in simul.split('[')[0] if n.isdigit()])
                if len(year)>=4: add = '.' + year[-4:] 
                else: add = ''
                if 'vrt' in simul or 'VRT' in simul:
                    self.his = folder + '/GIGATL6_5d_VRT.mean' + add
                elif 'KE' in simul or 'ke' in simul:
                ...
                self.his = folder + 'GIGATL6_3h_aver_'
                self.tfile=40
                self.dtfile=10800
                self.tend=100000
            elif 'avg' in simul:
                if 'vrt' in simul:
                    ...
                    self.his = folder + '/GIGATL6_5d_EDDY_'
                else:
                ...
                self.his = folder + 'GIGATL6_3h_inst_' #1999-01-25-1999-01-29'
                self.tfile=40
                self.dtfile=3*3600
                self.tend=10000
            else:
        ...
        ##################
            ...
            if 'avg' in simul:
                ...
                if 'mean' in simul:
                    ...
                    if 'vrt' in simul:
                        self.his = folder + '/rrexnum100_diags_vrt_avg.'
                    elif 'ek' in simul:
                        self.his = folder +  '/rrexnum100_diags_ek_avg.'
                    elif 'uv' in simul:
                        ...
                        self.his = folder + '/rrexnum100_diags_wdia_avg.'                    
                    else:
                        self.his = folder + '/rrexnum100_avg.'
            else:
                self.tfile=2
                if 'vrt' in simul:
                    self.his = folder +  '/rrexnum100_diags_vrt.'
                elif 'ek' in simul:
                    self.his = folder + '/rrexnum100_diags_ek.'
                elif 'uv' in simul:
                    ...
                    self.his = folder +  '/rrexnum100_diags_wdia.'
                else:
                    self.his = folder +  '/rrexnum100_his.'
...
# old version
        ...
        elif 'eurec1' in simul:
        
            self.realyear = True
            self.realyear_origin = datetime(1979,1,1)
            self.realyear_tstart = datetime(2011,8,1,12)

            self.model = 'croco_xios'
            self.digits = 0

            if os.getenv('HOSTNAME') is None:
                folder= libra +'/gula/ROMS/Simulations/EUREC1/'
                self.grd=folder + 'eurec1_grd.nc'
            else:
                folder= '/home/datawork-lops-osi/jgula/EUREC1'
                self.grd= folder + '/INIT/eurec1_grd.nc'


            if 'avg' in simul:
                self.his = folder + '/HIS/EUREC1_5d_aver_'
                self.tfile=1
                self.dtfile=432000
                self.tend=100000
            else:
                self.his = folder + '/HIS/EUREC1_1h_inst_'
                self.tfile=24
                self.dtfile=3600
                self.tend=100000
        ...
        ##################
            ...
            if os.getenv('HOSTNAME') is None:
                ...
                folder= '/home/datawork-lops-osi/jgula/BRAZIL/OUT/'
                self.grd= '/home/datawork-lops-osi/jgula/BRAZIL/INIT/brazil_grd.nc'


            if 'mean' in simul:
                if 'var' in simul:
                    self.his = folder + '/BRAZIL_12h_inst_2008_meanvaruv'
                else:
                ...
                self.his = folder + '/BRAZIL_5d_aver_'
                self.tfile=1
                self.dtfile=432000
                self.tend=100000
            else:
                self.his = folder + '/BRAZIL_12h_inst_'
                self.tfile=10
                self.dtfile=12*3600
                self.tend=100000
        ...
        ##################
            ...
            if os.getenv('HOSTNAME') is None:
                if 'master' in simul:
                    folder= libra +'/gula/ROMS/Simulations/PLAIN_master/'
                else:
                    folder= libra +'/gula/ROMS/Simulations/PLAIN/'
                self.grd=folder + 'plain_grd.nc'
            else:
                if 'master' in simul:
                    folder= '/home/datawork-lops-osi/jgula/PLAIN/HIS_nc4par_master/'
                else:
                    folder= '/home/datawork-lops-osi/jgula/PLAIN/HIS_nc4par_v2/'
                self.grd= '/home/datawork-lops-osi/jgula/PLAIN/INIT/plain_grd.nc'
            ...
            if 'mean' in simul:
                if 'var' in simul:
                    ...
                    self.his = folder + '/plain_diags_ek_avg.mean'
                elif 'uv' in simul:
                    self.his = folder + '/plain_diaM_avg.mean'
                elif 'ts' in simul:
                    self.his = folder + '/plain_diaT_avg.mean'
                elif 'pv' in simul:
                    self.his = folder + '/plain_diags_pv_avg.mean'
                elif 'wdia' in simul:
                    self.his = folder + '/plain_diags_wdia_avg.mean'
                elif 'eddy' in simul:
                    self.his = folder + '/plain_diags_eddy_avg.mean'
                else:
                    self.his = folder + '/plain_avg.mean'
                self.digits = 0
                self.model = 'croco'
                self.tfile=1; self.dtfile=1; self.tend=1
            elif 'avg' in simul:
                if 'vrt' in simul:
                    self.his = folder + '/plain_diags_vrt_avg.'
                elif 'ek' in simul:
                    self.his = folder +  '/plain_diags_ek_avg.'
                elif 'uv' in simul:
                    self.his = folder + '/plain_diaM_avg.'
                elif 'ts' in simul:
                    self.his = folder + '/plain_diaT_avg.'
                elif 'pv' in simul:
                    self.his = folder + '/plain_diags_pv_avg.'
                elif 'wdia' in simul:
                    self.his = folder + '/plain_diags_wdia_avg.'
                elif 'eddy' in simul:
                    self.his = folder + '/plain_diags_eddy_avg.'
                else:
                    self.his = folder + '/plain_avg.'
                self.tfile=1
                self.dtfile=432000
                self.tend=100000
            else:
                if 'vrt' in simul:
                    self.his = folder + '/plain_diags_vrt.'
                elif 'ek' in simul:
                    self.his = folder +  '/plain_diags_ek.'
                elif 'uv' in simul:
                    self.his = folder + '/plain_diaM.'
                elif 'ts' in simul:
                    self.his = folder + '/plain_diaT.'
                elif 'pv' in simul:
                    self.his = folder + '/plain_diags_pv.'
                elif 'wdia' in simul:
                    self.his = folder + '/plain_diags_wdia.'
                else:
                    self.his = folder + '/plain_his.'
                self.tfile=2
                self.dtfile=12*3600
                self.tend=100000
        ...
        ##################
            ...
            if os.getenv('HOSTNAME') is None:
                ...
                folder= '/home/datawork-lops-osi/jgula/GSUP5/'
            
            self.grd = folder + 'GS_2km_avg_Y2007M03T0.nc'
            self.his = folder + 'GS_2km_avg_Y2007M03T0'
            self.tfile=1
            self.dtfile=3*3600
            self.tend=100000
            self.tstart=0
