...
###################################################################################
# VARIABLES 
####################/Users/gula/Desktop/Work/Python/Python_Modules_p3/R_tools_gula.py###############################################################
...
###################################################################################
    ...
    def __init__(self,varname,simul,n2max=100000,method='new',verbo=False,**kwargs):
        ...
        # test for ['rho'] removed 17/11/24
            ...
            if verbo: print('varname',varname)
            if verbo: print('dimensions',ncfile.variables[varname].dimensions)
            if verbo: print('imin,jmin,kmin',self.imin,self.jmin,self.kmin)
            ...
            ####################################################################
                ...
                if verbo: print('It is a 3D variable on z-levels (interpolation needed)')
                ...
                for i,j in product(list(range(nchunk)),list(range(nchunk))):
                    ...
                    if verbo: print('compute z_r,z_w')
                    ...
                    if verbo: print('Compute variable in subdomain')
                    ...
                    if verbo:
                        print(simul.infiletime,ny1i+self.jper,ny2i-self.jmin+self.jper,nx1i+self.iper,nx2i-self.imin+self.iper)
                        myvar = simul.Forder(ncfile.variables[varname][simul.infiletime,:,ny1i+self.jper:ny2i-self.jmin+self.jper,nx1i+self.iper:nx2i-self.imin+self.iper])
                        print(myvar.shape)
                    
                    try:
                        mask = simul.mask[nx1i+self.iper-nx1:nx2i+self.iper-nx1, ny1i+self.jper-ny1:ny2i+self.jper-ny1]
                        #print(mask.shape)
                        #print(z_r.shape)
                        #print('use mask for vinterp')
                    except:
                        mask = None
        ...
        ####################################################################
            ...
            for i,j in product(list(range(nchunk)),list(range(nchunk))):
                ...
                elif (zsize==1) and (np.min(depths)==0) and (self.name in ['vrt','str','absvrt','ow','div']):
