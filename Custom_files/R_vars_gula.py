    ...
    def __init__(self,varname,simul,n2max=50000,method='new',debug=False,**kwargs):
        ...
        #check if variable is already in output:  
            ...
            ####################################################################
                ...
                for i,j in product(list(range(nchunk)),list(range(nchunk))):
                    ...
                    nx1i = np.int(nx1+i*(nx2-nx1)/nchunk-2*dx1)
                    nx2i = np.int(nx1+(i+1)*(nx2-nx1)/nchunk+2*dx2)
                    ny1i = np.int(ny1+j*(ny2-ny1)/nchunk-2*dy1)
                    ny2i = np.int(ny1+(j+1)*(ny2-ny1)/nchunk+2*dy2)
        ...
        ####################################################################
            ...
            for i,j in product(list(range(nchunk)),list(range(nchunk))):
                ...
                nx1i = np.int(nx1+i*(nx2-nx1)/nchunk-dx1)
                nx2i = np.int(nx1+(i+1)*(nx2-nx1)/nchunk+dx2)
                ny1i = np.int(ny1+j*(ny2-ny1)/nchunk-dy1)
                ny2i = np.int(ny1+(j+1)*(ny2-ny1)/nchunk+dy2)
